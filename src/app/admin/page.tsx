...
interface Order {
  ...
  products?: { name: string; price_usdt: number }
}
...
const S = {
  ...
  logo:    { display:'flex' as const, alignItems:'center' as const, gap:10, textDecoration:'none' as const },
  ...
  title:   { fontFamily:'Bebas Neue' as const, fontSize:24, letterSpacing:2, color:'#E8E8F0', marginBottom:4 },
  label:   { fontFamily:'DM Mono' as const, fontSize:10, color:'#4a4a6a', textTransform:'uppercase' as const, letterSpacing:2, marginBottom:12 },
  ...
  btn:     { background:'#6C47FF', border:'1px solid rgba(108,71,255,0.5)', borderRadius:8, padding:'10px 24px', color:'#fff', fontFamily:'Syne' as const, fontSize:13, fontWeight:700 as const, cursor:'pointer' as const, letterSpacing:0.5 },
  ...
  td:      { padding:'12px 12px', borderBottom:'1px solid rgba(108,71,255,0.06)', fontSize:12, color:'#8888a8', fontFamily:'DM Mono' as const, verticalAlign:'middle' as const },
}
...
export default function AdminPage() {
  ...
  const loadOrders = async (status: string) => {
    setLoading(true)
    const res = await fetch(`/api/verify?secret=${secret}&status=${status}`)
  ...
  const loadStats = async () => {
    ...
    const results = await Promise.all(
      statuses.map(s => fetch(`/api/verify?secret=${secret}&status=${s}`).then(r => r.json()))
    )
    ...
    setStats({
      total: totalOrders,
      pending: (pend.orders?.length || 0) + (verif.orders?.length || 0),
      confirmed: conf.orders?.length || 0,
      revenue,
    })
  }

  const handleLogin = async () => {
    const res = await fetch(`/api/verify?secret=${secret}&status=verifying`)
  ...
  const handleAction = async (orderId: string, action: 'confirmed' | 'rejected') => {
    const res = await fetch('/api/verify', {
      ...
      body: JSON.stringify({ order_id: orderId, action, admin_secret: secret }),
    })
    const data = await res.json()
    if (data.success) {
      showToast(action === 'confirmed' ? '✅ Pago confirmado — descarga habilitada' : '❌ Orden rechazada')
      if (action === 'confirmed' && data.download_url) {
        navigator.clipboard.writeText(data.download_url)
        showToast('✅ Confirmado · URL de descarga copiada al portapapeles')
      }
      loadOrders(filter)
      loadStats()
    } else {
  ...
  const changeFilter = (f: string) => { setFilter(f); loadOrders(f) }

  if (!auth) return (
    <div style={{ ...S.page, display:'flex', alignItems:'center', justifyContent:'center' }}>
      <div style={{ ...S.card, maxWidth:400, width:'100%', textAlign:'center' }}>
        <div style={{ fontSize:40, marginBottom:16 }}>🔐</div>
        <div style={{ ...S.title, textAlign:'center', marginBottom:24 }}>PANEL ADMIN</div>
        <div style={{ ...S.label, textAlign:'left' }}>Contraseña de acceso</div>
        <input
          ...
          placeholder="••••••••••••"
        />
        <button style={{ ...S.btn, width:'100%' }} onClick={handleLogin}>
          Entrar al panel
        </button>
        <div style={{ marginTop:16 }}>
          <Link href="/" style={{ fontFamily:'DM Mono', fontSize:11, color:'#4a4a6a' }}>← Volver al marketplace</Link>
        </div>
      </div>
    </div>
  )
    ...
    <div style={S.page}>
      ...
      <header style={S.nav}>
        <Link href="/" style={S.logo}>
          ...
          <span style={{ fontFamily:'DM Mono', fontSize:10, color:'#4a4a6a', marginLeft:4 }}>/ ADMIN</span>
        </Link>
      ...
      <div style={S.main}>
        ...
        <div style={S.card}>
            ...
            <div style={{ display:'flex', gap:6 }}>
              {['verifying','pending','confirmed','rejected'].map(f => (
                <button
                  ...
                  style={{
                    ...S.btnSm,
                    background: filter === f ? '#6C47FF' : 'rgba(108,71,255,0.08)',
                    color: filter === f ? '#fff' : '#8888a8',
                    border: `1px solid ${filter === f ? 'rgba(108,71,255,0.5)' : 'rgba(108,71,255,0.15)'}`,
              ...
              <button
                ...
                style={{ ...S.btnSm, background:'rgba(0,212,170,0.1)', color:'#00D4AA', border:'1px solid rgba(0,212,170,0.2)' }}
              >↻ Actualizar</button>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div style={{ textAlign:'center', padding:40, fontFamily:'DM Mono', fontSize:12, color:'#4a4a6a' }}>No hay órdenes con estado "{filter}"</div>
          ) : (
            <div style={{ overflowX:'auto' as const }}>
              <table style={S.table}>
                <thead>
                  <tr>
                    {['Fecha','Cliente','Producto','Monto','Red','TX Hash','Estado','Acciones'].map(h => (
                ...
                <tbody>
                  {orders.map(order => (
                    <tr key={order.id} style={{ transition:'background .15s' }}>
                      ...
                      <td style={S.td}>
                        <div style={{ color:'#E8E8F0' }}>{order.buyer_name || '—'}</div>
                        <div style={{ fontSize:10, color:'#4a4a6a' }}>{order.buyer_email}</div>
                      </td>
                      ...
                      <td style={S.td}>
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          style={{ color:'#6C47FF', textDecoration:'none', fontSize:11 }}
                        >
                      ...
                      <td style={S.td}>
                        <span style={{
                          ...S.btnSm,
                          cursor:'default',
                          background: STATUS_COLORS[order.status]?.bg || 'transparent',
                          color: STATUS_COLORS[order.status]?.color || '#8888a8',
                          border: `1px solid ${STATUS_COLORS[order.status]?.color || '#8888a8'}30`,
                          padding:'3px 10px',
                        }}>
                          {order.status}
                        </span>
                        ...
                        {order.status === 'verifying' && (
                          <div style={{ display:'flex', gap:6 }}>
                            <button
                              ...
                              style={{ ...S.btnSm, background:'rgba(0,212,170,0.15)', color:'#00D4AA', border:'1px solid rgba(0,212,170,0.3)' }}
                            >✓ Confirmar</button>
                            <button
                              onClick={() => handleAction(order.id, 'rejected')}
                              style={{ ...S.btnSm, background:'rgba(255,59,92,0.12)', color:'#FF3B5C', border:'1px solid rgba(255,59,92,0.25)' }}
                            >✗ Rechazar</button>
                          </div>
                        )}
                        {order.status === 'confirmed' && (
                          <button
                            onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/download/${order.download_token}`); showToast('URL copiada') }}
                            style={{ ...S.btnSm, background:'rgba(108,71,255,0.12)', color:'#A78BFA', border:'1px solid rgba(108,71,255,0.25)' }}
                          >📋 Copiar URL</button>
                        )}
                      </td>
                    </tr>
      ...
      {/* TOAST */}
      {toast && (
        <div style={{ position:'fixed', bottom:24, right:24, background:'#13132a', border:'1px solid rgba(108,71,255,0.3)', borderRadius:10, padding:'12px 18px', fontFamily:'DM Mono', fontSize:12, color:'#E8E8F0', zIndex:9999, boxShadow:'0 8px 32px rgba(0,0,0,0.4)' }}>
          {toast}
        </div>
      )}
    </div>
